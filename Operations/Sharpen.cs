...
public class Sharpen : IOperation
{
    ...
    public static Sharpen CreateFromUnityWorld(UltimateTerrain uTerrain, Vector3 position, float radius, double step)
    {
        var conv = uTerrain.Converter;
        return new Sharpen(uTerrain, conv.UnityToVoxelPosition(position), conv.UnityToVoxelDisance(radius), step);
    }
    ...
    {
        return string.Format("[Sharpen] at " + position + " of radius " + radius);
    }
