...
public class Capsule : IOperation
{
    ...
    // 
    public Capsule(UltimateTerrain uTerrain, bool dig, Vector3d start, Vector3d end, double radius)
    {
        this.dig = dig;
        this.start = start;
        this.end = end;
    ...
    // 
    public Capsule(UltimateTerrain uTerrain, bool dig, Vector3d start, Vector3d end, double radius, VoxelType voxelType) : this(uTerrain, dig, start, end, radius)
    {
        if (voxelType != null) {
    ...
    public static Capsule CreateFromUnityWorld(UltimateTerrain uTerrain, bool dig, Vector3 start, Vector3 end, float radius, VoxelType voxelType)
    {
        var conv = uTerrain.Converter;
        return new Capsule(uTerrain, dig, conv.UnityToVoxelPosition(start), conv.UnityToVoxelPosition(end), conv.UnityToVoxelDisance(radius), voxelType);
    }
